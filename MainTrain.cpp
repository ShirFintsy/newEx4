    ...
    if (c.feature1 == f1) {
        if (c.feature2 != f2)
            ...
            if (c.corrlation >= 0.9) {
                ...
                if (c.linReg.a < a - 0.5f || c.linReg.a > a + 0.5f)
                    cout << f1 << "-" << f2 << " wrong value of line_reg.a (-4)" << endl;
                if (c.linReg.b < b - 0.5f || c.linReg.b > b + 0.5f)
...
int main() {
    ...
    cout <<"all cor:" << cf.size()<< endl;
    if (cf.size() != 3)
        cout << "wrong size of correlated features (-40) - found: " << cf.size() << endl;
    else
    ...
    for (int i = 0; i < 3; i++)
        if (!detected[i])
            cout << "an anomaly was not detected (-10) " << i << endl;
        else
            falseAlarms--;
